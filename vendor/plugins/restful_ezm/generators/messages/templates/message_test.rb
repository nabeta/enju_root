...
class MessageTest < Test::Unit::TestCase
  ...
  def test_should_mark_message_read
    m = messages(:abby_to_sam_1)
    assert_nil m.read_at
    m.mark_message_read(users(:sam))
    assert_not_nil m.read_at
  end

  def test_should_purge_message
    m = messages(:abby_to_sam_Trash_2)
    n = m.id
    assert_not_nil m
    m.purge
  ...
  def create_message(options = {})
    Message.create({ :recipient => users(:sam), :sender => users(:abby), :subject => 'new message', :body => 'new message body is really short' }.merge(options))
  end
end
