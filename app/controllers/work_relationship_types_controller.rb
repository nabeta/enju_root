class WorkRelationshipTypesController < ApplicationController
  ...
  def index
    @work_relationship_types = WorkRelationshipType.find(:all)
  ...
  def create
    ...
    respond_to do |format|
      if @work_relationship_type.save
        flash[:notice] = 'WorkRelationshipType was successfully created.'
  ...
  def update
    ...
    respond_to do |format|
      if @work_relationship_type.update_attributes(params[:work_relationship_type])
        flash[:notice] = 'WorkRelationshipType was successfully updated.'
        format.html { redirect_to(@work_relationship_type) }
        format.xml  { head :ok }
      else
