class PatronMergesController < ApplicationController
  ...
  def index
    if @patron
      @patron_merges = @patron.patron_merges.find(:all, :order => ['patron_merges.id'])
    elsif @patron_merge_list
      @patron_merges = @patron_merge_list.patron_merges.find(:all, :include => 'patron', :order => ['patron_merges.id'])
    else
      @patron_merges = PatronMerge.find(:all)
    end
