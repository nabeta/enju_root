class ExpressionMergesController < ApplicationController
  ...
  def index
    if @expression
      @expression_merges = @expression.expression_merges.paginate(:all, :page => params[:page], :order => ['expression_merges.id'])
    elsif @expression_merge_list
      @expression_merges = @expression_merge_list.expression_merges.paginate(:all, :page => params[:page], :include => 'expression', :order => ['expression_merges.id'])
    else
      @expression_merges = ExpressionMerge.paginate(:all, :page => params[:page])
    end
