class ItemHasUseRestrictionsController < ApplicationController
  ...
  def index
    if @item
      @item_has_use_restrictions = @item.item_has_use_restrictions.paginate(:page => params[:page], :per_page => @per_page, :order => ['item_has_use_restrictions.id'])
    else
      @item_has_use_restrictions = ItemHasUseRestriction.paginate(:page => params[:page], :per_page => @per_page, :order => :id)
    end
  ...
  def create
    ...
    respond_to do |format|
      if @item_has_use_restriction.save
        flash[:notice] = ('ItemHasUseRestriction was successfully created.')
  ...
  def update
    ...
    respond_to do |format|
      if @item_has_use_restriction.update_attributes(params[:item_has_use_restriction])
        flash[:notice] = ('ItemHasUseRestriction was successfully updated.')
        format.html { redirect_to(@item_has_use_restriction) }
        format.xml  { head :ok }
      else
        @use_restrictions = UseRestriction.find(:all)
