...
xml.tag! "OAI-PMH", :xmlns => "http://www.openarchives.org/OAI/2.0/",
  ...
  xml.ListRecords do
    @manifestations.each do |manifestation|
      cache(:id => manifestation.id, :action => 'show', :controller => 'manifestations', :role => current_user.try(:highest_role).try(:name), :format_suffix => 'oai_pmh_list_records', :locale => @locale) do
        xml.record do
          xml.header do
