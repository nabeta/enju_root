...
xml.rss('version' => "2.0",
  ...
  xml.channel{
    if @user
      xml.title t('message.user_message', :login_name => @user.login)
      xml.link user_messages_url(@user.login, :format => :rss)
    else
    ...
    if @user
      ...
      xml.tag! "atom:link", :rel => 'alternate', :href => user_messages_url(@user.login, :format => :rss)
    else
      xml.tag! "atom:link", :rel => 'self', :href => messages_url(:format => :rss)
